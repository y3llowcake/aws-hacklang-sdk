...
interface ElasticTranscoder {
  ...
  public function DeletePipeline(DeletePipelineRequest $in): Awaitable<\Errors\Result<DeletePipelineResponse>>;
  public function DeletePreset(DeletePresetRequest $in): Awaitable<\Errors\Result<DeletePresetResponse>>;
  public function ListJobsByPipeline(ListJobsByPipelineRequest $in): Awaitable<\Errors\Result<ListJobsByPipelineResponse>>;
  public function ListJobsByStatus(ListJobsByStatusRequest $in): Awaitable<\Errors\Result<ListJobsByStatusResponse>>;
  ...
  public function ReadJob(ReadJobRequest $in): Awaitable<\Errors\Result<ReadJobResponse>>;
  public function ReadPipeline(ReadPipelineRequest $in): Awaitable<\Errors\Result<ReadPipelineResponse>>;
  public function ReadPreset(ReadPresetRequest $in): Awaitable<\Errors\Result<ReadPresetResponse>>;
  public function TestRole(TestRoleRequest $in): Awaitable<\Errors\Result<TestRoleResponse>>;
  public function UpdatePipeline(UpdatePipelineRequest $in): Awaitable<\Errors\Result<UpdatePipelineResponse>>;
  public function UpdatePipelineNotifications(UpdatePipelineNotificationsRequest $in): Awaitable<\Errors\Result<UpdatePipelineNotificationsResponse>>;
  public function UpdatePipelineStatus(UpdatePipelineStatusRequest $in): Awaitable<\Errors\Result<UpdatePipelineStatusResponse>>;
}
...
class Artwork {
  ...
  public function __construct(shape(
    ...
    $this->album_art_format = $s['album_art_format'] ?? '';
    $this->encryption = $s['encryption'] ?? null;
    $this->input_key = $s['input_key'] ?? '';
    $this->max_height = $s['max_height'] ?? '';
    $this->max_width = $s['max_width'] ?? '';
    $this->padding_policy = $s['padding_policy'] ?? '';
    $this->sizing_policy = $s['sizing_policy'] ?? '';
  }
}
...
class AudioCodecOptions {
  ...
  public function __construct(shape(
    ...
    $this->bit_depth = $s['bit_depth'] ?? '';
    $this->bit_order = $s['bit_order'] ?? '';
...
class AudioParameters {
  ...
  public function __construct(shape(
    ...
    $this->audio_packing_mode = $s['audio_packing_mode'] ?? '';
    $this->bit_rate = $s['bit_rate'] ?? '';
    $this->channels = $s['channels'] ?? '';
    $this->codec = $s['codec'] ?? '';
    $this->codec_options = $s['codec_options'] ?? null;
...
class CaptionSource {
  ...
  public function __construct(shape(
    ...
    $this->time_offset = $s['time_offset'] ?? '';
  }
}
...
class Captions {
  ...
  public function __construct(shape(
    ...
    $this->caption_formats = $s['caption_formats'] ?? vec[];
    $this->caption_sources = $s['caption_sources'] ?? vec[];
    $this->merge_policy = $s['merge_policy'] ?? '';
  }
}
...
class Clip {
  ...
  public function __construct(shape(
    ...
    $this->time_span = $s['time_span'] ?? null;
  }
}
...
class CreateJobOutput {
  ...
  public function __construct(shape(
    ...
    $this->album_art = $s['album_art'] ?? null;
    $this->captions = $s['captions'] ?? null;
    $this->composition = $s['composition'] ?? vec[];
    $this->encryption = $s['encryption'] ?? null;
    $this->key = $s['key'] ?? '';
    $this->preset_id = $s['preset_id'] ?? '';
    $this->rotate = $s['rotate'] ?? '';
    $this->segment_duration = $s['segment_duration'] ?? '';
    $this->thumbnail_encryption = $s['thumbnail_encryption'] ?? null;
    $this->thumbnail_pattern = $s['thumbnail_pattern'] ?? '';
    $this->watermarks = $s['watermarks'] ?? vec[];
  }
}
...
class CreateJobPlaylist {
  ...
  public function __construct(shape(
    ...
    $this->format = $s['format'] ?? '';
    $this->hls_content_protection = $s['hls_content_protection'] ?? null;
    $this->name = $s['name'] ?? '';
    $this->output_keys = $s['output_keys'] ?? vec[];
    $this->play_ready_drm = $s['play_ready_drm'] ?? null;
  }
}
...
class CreateJobRequest {
  ...
  public function __construct(shape(
    ...
    $this->output_key_prefix = $s['output_key_prefix'] ?? '';
    $this->outputs = $s['outputs'] ?? vec[];
    $this->pipeline_id = $s['pipeline_id'] ?? '';
    $this->playlists = $s['playlists'] ?? vec[];
...
class CreatePipelineRequest {
  ...
  public function __construct(shape(
    ...
    $this->aws_kms_key_arn = $s['aws_kms_key_arn'] ?? '';
    $this->content_config = $s['content_config'] ?? null;
    $this->input_bucket = $s['input_bucket'] ?? '';
    $this->name = $s['name'] ?? '';
    $this->notifications = $s['notifications'] ?? null;
    $this->output_bucket = $s['output_bucket'] ?? '';
    $this->role = $s['role'] ?? '';
    $this->thumbnail_config = $s['thumbnail_config'] ?? null;
  }
}
...
class CreatePresetRequest {
  ...
  public function __construct(shape(
    ...
    $this->audio = $s['audio'] ?? null;
    $this->container = $s['container'] ?? '';
    $this->description = $s['description'] ?? '';
    $this->name = $s['name'] ?? '';
    $this->thumbnails = $s['thumbnails'] ?? null;
...
class HlsContentProtection {
  ...
  public function __construct(shape(
    ...
    $this->key = $s['key'] ?? '';
    $this->key_md_5 = $s['key_md_5'] ?? '';
    $this->key_storage_policy = $s['key_storage_policy'] ?? '';
    $this->license_acquisition_url = $s['license_acquisition_url'] ?? '';
...
class JobInput {
  ...
  public function __construct(shape(
    ...
    $this->aspect_ratio = $s['aspect_ratio'] ?? '';
    $this->container = $s['container'] ?? '';
    $this->detected_properties = $s['detected_properties'] ?? null;
    $this->encryption = $s['encryption'] ?? null;
    $this->frame_rate = $s['frame_rate'] ?? '';
    $this->input_captions = $s['input_captions'] ?? null;
...
class JobOutput {
  ...
  public function __construct(shape(
    ...
    $this->album_art = $s['album_art'] ?? null;
    $this->applied_color_space_conversion = $s['applied_color_space_conversion'] ?? '';
    $this->captions = $s['captions'] ?? null;
    $this->composition = $s['composition'] ?? vec[];
...
class JobWatermark {
  ...
  public function __construct(shape(
    ...
    $this->encryption = $s['encryption'] ?? null;
    $this->input_key = $s['input_key'] ?? '';
    $this->preset_watermark_id = $s['preset_watermark_id'] ?? '';
  }
}
...
class Notifications {
  ...
  public function __construct(shape(
    ...
    $this->error = $s['error'] ?? '';
    $this->progressing = $s['progressing'] ?? '';
    $this->warning = $s['warning'] ?? '';
  }
}
...
class Permission {
  ...
  public function __construct(shape(
    ...
    $this->access = $s['access'] ?? vec[];
    $this->grantee = $s['grantee'] ?? '';
...
class PresetWatermark {
  ...
  public function __construct(shape(
    ...
    $this->horizontal_align = $s['horizontal_align'] ?? '';
    $this->horizontal_offset = $s['horizontal_offset'] ?? '';
    $this->id = $s['id'] ?? '';
    $this->max_height = $s['max_height'] ?? '';
    $this->max_width = $s['max_width'] ?? '';
    ...
    $this->vertical_align = $s['vertical_align'] ?? '';
    $this->vertical_offset = $s['vertical_offset'] ?? '';
  }
}
...
class Timing {
  ...
  public function __construct(shape(
    ...
    $this->finish_time_millis = $s['finish_time_millis'] ?? 0;
    $this->start_time_millis = $s['start_time_millis'] ?? 0;
    $this->submit_time_millis = $s['submit_time_millis'] ?? 0;
  }
}
...
class VideoParameters {
  ...
  public function __construct(shape(
    ...
    $this->bit_rate = $s['bit_rate'] ?? '';
    $this->codec = $s['codec'] ?? '';
    $this->codec_options = $s['codec_options'] ?? dict[];
    $this->display_aspect_ratio = $s['display_aspect_ratio'] ?? '';
    $this->fixed_gop = $s['fixed_gop'] ?? '';
    $this->frame_rate = $s['frame_rate'] ?? '';
    $this->keyframes_max_dist = $s['keyframes_max_dist'] ?? '';
    $this->max_frame_rate = $s['max_frame_rate'] ?? '';
    $this->max_height = $s['max_height'] ?? '';
    $this->max_width = $s['max_width'] ?? '';
    $this->padding_policy = $s['padding_policy'] ?? '';
    $this->resolution = $s['resolution'] ?? '';
