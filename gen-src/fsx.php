...
interface FSx {
  public function CancelDataRepositoryTask(CancelDataRepositoryTaskRequest $in): Awaitable<\Errors\Result<CancelDataRepositoryTaskResponse>>;
  public function CreateBackup(CreateBackupRequest $in): Awaitable<\Errors\Result<CreateBackupResponse>>;
  public function CreateDataRepositoryTask(CreateDataRepositoryTaskRequest $in): Awaitable<\Errors\Result<CreateDataRepositoryTaskResponse>>;
  public function CreateFileSystem(CreateFileSystemRequest $in): Awaitable<\Errors\Result<CreateFileSystemResponse>>;
  public function CreateFileSystemFromBackup(CreateFileSystemFromBackupRequest $in): Awaitable<\Errors\Result<CreateFileSystemFromBackupResponse>>;
  public function DeleteBackup(DeleteBackupRequest $in): Awaitable<\Errors\Result<DeleteBackupResponse>>;
  public function DeleteFileSystem(DeleteFileSystemRequest $in): Awaitable<\Errors\Result<DeleteFileSystemResponse>>;
  public function DescribeBackups(DescribeBackupsRequest $in): Awaitable<\Errors\Result<DescribeBackupsResponse>>;
  public function DescribeDataRepositoryTasks(DescribeDataRepositoryTasksRequest $in): Awaitable<\Errors\Result<DescribeDataRepositoryTasksResponse>>;
  ...
  public function UpdateFileSystem(UpdateFileSystemRequest $in): Awaitable<\Errors\Result<UpdateFileSystemResponse>>;
}
...
class ActiveDirectoryBackupAttributes {
  ...
  public function __construct(shape(
    ...
    $this->active_directory_id = $s['active_directory_id'] ?? '';
...
class Backup {
  ...
  public function __construct(shape(
    ...
    $this->directory_information = $s['directory_information'] ?? null;
    $this->failure_details = $s['failure_details'] ?? null;
    $this->file_system = $s['file_system'] ?? null;
...
class CreateDataRepositoryTaskResponse {
  ...
  public function __construct(shape(
    ...
    $this->data_repository_task = $s['data_repository_task'] ?? null;
  }
}
...
class CreateFileSystemFromBackupRequest {
  ...
  public function __construct(shape(
    ...
    $this->tags = $s['tags'] ?? vec[];
    $this->windows_configuration = $s['windows_configuration'] ?? null;
  }
}
...
class CreateFileSystemLustreConfiguration {
  ...
  public function __construct(shape(
    ...
    $this->export_path = $s['export_path'] ?? '';
    $this->import_path = $s['import_path'] ?? '';
    $this->imported_file_chunk_size = $s['imported_file_chunk_size'] ?? 0;
    $this->per_unit_storage_throughput = $s['per_unit_storage_throughput'] ?? 0;
    $this->weekly_maintenance_start_time = $s['weekly_maintenance_start_time'] ?? '';
  }
}
...
class CreateFileSystemRequest {
  ...
  public function __construct(shape(
    ...
    $this->kms_key_id = $s['kms_key_id'] ?? '';
    $this->lustre_configuration = $s['lustre_configuration'] ?? null;
    $this->security_group_ids = $s['security_group_ids'] ?? vec[];
    $this->storage_capacity = $s['storage_capacity'] ?? 0;
    $this->storage_type = $s['storage_type'] ?? '';
    $this->subnet_ids = $s['subnet_ids'] ?? vec[];
    $this->tags = $s['tags'] ?? vec[];
    $this->windows_configuration = $s['windows_configuration'] ?? null;
...
class CreateFileSystemWindowsConfiguration {
  ...
  public function __construct(shape(
    ...
    $this->active_directory_id = $s['active_directory_id'] ?? '';
    $this->automatic_backup_retention_days = $s['automatic_backup_retention_days'] ?? 0;
    $this->copy_tags_to_backups = $s['copy_tags_to_backups'] ?? false;
    $this->daily_automatic_backup_start_time = $s['daily_automatic_backup_start_time'] ?? '';
    $this->deployment_type = $s['deployment_type'] ?? '';
    $this->preferred_subnet_id = $s['preferred_subnet_id'] ?? '';
    $this->self_managed_active_directory_configuration = $s['self_managed_active_directory_configuration'] ?? null;
    $this->throughput_capacity = $s['throughput_capacity'] ?? 0;
    $this->weekly_maintenance_start_time = $s['weekly_maintenance_start_time'] ?? '';
  }
}
...
class DataRepositoryTaskStatus {
  ...
  public function __construct(shape(
    ...
    $this->succeeded_count = $s['succeeded_count'] ?? 0;
...
class DeleteFileSystemResponse {
  ...
  public function __construct(shape(
    ...
    $this->file_system_id = $s['file_system_id'] ?? '';
    $this->lifecycle = $s['lifecycle'] ?? '';
    $this->windows_response = $s['windows_response'] ?? null;
  }
}
...
class DeleteFileSystemWindowsConfiguration {
  ...
  public function __construct(shape(
    ...
    $this->final_backup_tags = $s['final_backup_tags'] ?? vec[];
    $this->skip_final_backup = $s['skip_final_backup'] ?? false;
  }
}
...
class DeleteFileSystemWindowsResponse {
  ...
  public function __construct(shape(
    ...
    $this->final_backup_id = $s['final_backup_id'] ?? '';
    $this->final_backup_tags = $s['final_backup_tags'] ?? vec[];
  }
}
...
class DescribeBackupsRequest {
  ...
  public function __construct(shape(
    ...
    $this->backup_ids = $s['backup_ids'] ?? vec[];
...
class DescribeDataRepositoryTasksResponse {
  ...
  public function __construct(shape(
    ...
    $this->data_repository_tasks = $s['data_repository_tasks'] ?? vec[];
    $this->next_token = $s['next_token'] ?? '';
  }
}
...
class DescribeFileSystemsRequest {
  ...
  public function __construct(shape(
    ...
    $this->file_system_ids = $s['file_system_ids'] ?? vec[];
    $this->max_results = $s['max_results'] ?? 0;
    $this->next_token = $s['next_token'] ?? '';
  }
}
...
class InvalidNetworkSettings {
  ...
  public function __construct(shape(
    ...
    $this->invalid_security_group_id = $s['invalid_security_group_id'] ?? '';
    $this->invalid_subnet_id = $s['invalid_subnet_id'] ?? '';
    $this->message = $s['message'] ?? '';
  }
}
...
class LustreFileSystemConfiguration {
  ...
  public function __construct(shape(
    ...
    $this->data_repository_configuration = $s['data_repository_configuration'] ?? null;
    $this->deployment_type = $s['deployment_type'] ?? '';
    $this->mount_name = $s['mount_name'] ?? '';
    $this->per_unit_storage_throughput = $s['per_unit_storage_throughput'] ?? 0;
    $this->weekly_maintenance_start_time = $s['weekly_maintenance_start_time'] ?? '';
  }
}
...
class SelfManagedActiveDirectoryAttributes {
  ...
  public function __construct(shape(
    ...
    $this->dns_ips = $s['dns_ips'] ?? vec[];
    $this->domain_name = $s['domain_name'] ?? '';
    $this->file_system_administrators_group = $s['file_system_administrators_group'] ?? '';
...
class WindowsFileSystemConfiguration {
  ...
  public function __construct(shape(
    ...
    $this->daily_automatic_backup_start_time = $s['daily_automatic_backup_start_time'] ?? '';
    $this->deployment_type = $s['deployment_type'] ?? '';
    $this->maintenance_operations_in_progress = $s['maintenance_operations_in_progress'] ?? vec[];
    $this->preferred_file_server_ip = $s['preferred_file_server_ip'] ?? '';
    $this->preferred_subnet_id = $s['preferred_subnet_id'] ?? '';
    $this->remote_administration_endpoint = $s['remote_administration_endpoint'] ?? '';
    $this->self_managed_active_directory_configuration = $s['self_managed_active_directory_configuration'] ?? null;
    $this->throughput_capacity = $s['throughput_capacity'] ?? 0;
    $this->weekly_maintenance_start_time = $s['weekly_maintenance_start_time'] ?? '';
  }
