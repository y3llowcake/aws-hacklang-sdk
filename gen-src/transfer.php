...
interface Transfer {
  ...
  public function DeleteServer(DeleteServerRequest $in): Awaitable<\Errors\Error>;
  ...
  public function DescribeServer(DescribeServerRequest $in): Awaitable<\Errors\Result<DescribeServerResponse>>;
  public function DescribeUser(DescribeUserRequest $in): Awaitable<\Errors\Result<DescribeUserResponse>>;
  public function ImportSshPublicKey(ImportSshPublicKeyRequest $in): Awaitable<\Errors\Result<ImportSshPublicKeyResponse>>;
  public function ListServers(ListServersRequest $in): Awaitable<\Errors\Result<ListServersResponse>>;
  public function ListTagsForResource(ListTagsForResourceRequest $in): Awaitable<\Errors\Result<ListTagsForResourceResponse>>;
  public function ListUsers(ListUsersRequest $in): Awaitable<\Errors\Result<ListUsersResponse>>;
  public function StartServer(StartServerRequest $in): Awaitable<\Errors\Error>;
  public function StopServer(StopServerRequest $in): Awaitable<\Errors\Error>;
  public function TagResource(TagResourceRequest $in): Awaitable<\Errors\Error>;
  public function TestIdentityProvider(TestIdentityProviderRequest $in): Awaitable<\Errors\Result<TestIdentityProviderResponse>>;
...
class CreateServerRequest {
  ...
  public function __construct(shape(
    ...
    $this->endpoint_details = $s['endpoint_details'] ?? null;
    $this->endpoint_type = $s['endpoint_type'] ?? '';
    $this->host_key = $s['host_key'] ?? '';
    $this->identity_provider_details = $s['identity_provider_details'] ?? null;
    $this->identity_provider_type = $s['identity_provider_type'] ?? '';
    $this->logging_role = $s['logging_role'] ?? '';
...
class CreateUserRequest {
  ...
  public function __construct(shape(
    ...
    $this->home_directory = $s['home_directory'] ?? '';
    $this->home_directory_mappings = $s['home_directory_mappings'] ?? vec[];
    $this->home_directory_type = $s['home_directory_type'] ?? '';
...
class DescribedServer {
  ...
  public function __construct(shape(
    ...
    $this->endpoint_details = $s['endpoint_details'] ?? null;
    $this->endpoint_type = $s['endpoint_type'] ?? '';
    $this->host_key_fingerprint = $s['host_key_fingerprint'] ?? '';
    $this->identity_provider_details = $s['identity_provider_details'] ?? null;
    $this->identity_provider_type = $s['identity_provider_type'] ?? '';
    $this->logging_role = $s['logging_role'] ?? '';
    $this->protocols = $s['protocols'] ?? vec[];
    ...
    $this->state = $s['state'] ?? '';
    $this->tags = $s['tags'] ?? vec[];
    $this->user_count = $s['user_count'] ?? 0;
  }
}
...
class EndpointDetails {
  ...
  public function __construct(shape(
    ...
    $this->address_allocation_ids = $s['address_allocation_ids'] ?? vec[];
...
class ListedUser {
  ...
  public function __construct(shape(
    ...
    $this->home_directory_type = $s['home_directory_type'] ?? '';
    $this->role = $s['role'] ?? '';
    $this->ssh_public_key_count = $s['ssh_public_key_count'] ?? 0;
    $this->user_name = $s['user_name'] ?? '';
  }
}
...
class SshPublicKey {
  ...
  public function __construct(shape(
    ...
    $this->date_imported = $s['date_imported'] ?? 0;
    $this->ssh_public_key_body = $s['ssh_public_key_body'] ?? '';
    $this->ssh_public_key_id = $s['ssh_public_key_id'] ?? '';
  }
}
...
class TestIdentityProviderRequest {
  ...
  public function __construct(shape(
    ...
    $this->server_id = $s['server_id'] ?? '';
    $this->server_protocol = $s['server_protocol'] ?? '';
    $this->user_name = $s['user_name'] ?? '';
